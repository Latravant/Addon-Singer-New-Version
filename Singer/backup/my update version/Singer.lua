...
function do_stuff()
    ...
    if counter >= del then
        ...
        if clarion == true and windower.ffxi.get_ability_recasts()[get.ability_by_name('Clarion Call').recast_id] == 0 then
            windower.send_command('/ja "Clarion Call" <me>')
            del = 1.5
            return
        end
        
        if soul_voice == true and windower.ffxi.get_ability_recasts()[get.ability_by_name('Soul Voice').recast_id] == 0 then
