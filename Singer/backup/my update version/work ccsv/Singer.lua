...
local clarion = res.job_abilities:with('en', 'Clarion Call') -- يحضر Clarion Call من الجدول
res.job_abilities:with('en', 'Soul Voice')

res.job_abilities:with('en', 'Clarion Call')
...
local buttons = {'active','actions','nightingale','troubadour','pianissimo','clarion','soul_voice','debuffing','party','p1','p2','p3','p4','p5'}
...
function do_stuff()
    ...
    if counter >= del then
        ...
        local clarion = res.job_abilities:with('en', 'Clarion Call') -- يحضر Clarion Call من الجدول
        local soul_voice = res.job_abilities:with('en', 'Soul Voice') -- يحضر Clarion Call من الجدول

        res.job_abilities:with('en', 'Soul Voice')

        res.job_abilities:with('en', 'Clarion Call')
        if clarion and windower.ffxi.get_ability_recasts()[clarion.recast_id] == 0 then
            windower.send_command('input /ja "Clarion Call" <me>')
            del = 1.5
            return
        end        
        if soul_voice and windower.ffxi.get_ability_recasts()[soul_voice.recast_id] == 0 then
            windower.send_command('input /ja "Soul Voice" <me>')
            del = 1.5
            return
        end
        -- if not buffs['Soul Voice'] and windower.ffxi.get_ability_recasts()[get.ability_by_name('Soul Voice').recast_id] == 0 then
        --     windower.send_command('/ja "Soul Voice" <me>')
        --     del = 1.5
        --     return
        -- end
